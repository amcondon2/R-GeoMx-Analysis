...
```{r}
splsdaResult <- splsda(summedCountsMatrixPCKP, as.factor(joined$Group.x), keepX = 50, scale = TRUE, ncomp=2)
...
plotGeneWrtFlimFeature <- function(seObject, flimParamsTable, geneName, colName, yLabel) {
  ...
  toPlot <- cbind(flimFeature, geneCounts, subset(joined, select = c(sample, Group.x, Sex.x)))
  
  flimCol <- ensym(colName)
  ggplot(toPlot, aes(x=geneCounts, y=!!ensym(colName), color=Group.x, shape=Sex.x)) + geom_point() + theme_bw() +
    xlab("Log Counts") + 
    ylab(yLabel) + 
    ggtitle(geneName) +
    labs(shape = "Sex")
...
fviz_pca_biplot(res.pca, col.ind = joined$Group.x, repel = TRUE)
