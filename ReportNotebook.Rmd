...
de_genes_toptable_BvT %>% 
  ...
  DT::datatable(caption = 'Treatment vs. Control, PanCK+ Segments (limma-voom)') %>%
...
EnhancedVolcano(de_genes_toptable_BvT,
                lab = de_genes_toptable_BvT$TargetName,
                x='logFC',
                y='adj.P.Val', labSize = 4, drawConnectors = TRUE) + theme_bw(base_size = 10)

print(length(de_genes_toptable_BvT %>% filter(between(logFC, -1, 1)) %>% filter(between(adj.P.Val, 0, 0.05))))
```
...
getPvalsPathway <- function(upSetName, setCollection, rankedGenes, singscoreObject, numPerms) {
  #browser()
  nullResult <- generateNull(setCollection[[upSetName]], rankData = rankedGenes, knownDirection = FALSE)
  
  scoredf <- as.data.frame(cbind(singscoreObject$Scores[upSetName,],singscoreObject$Dispersions[upSetName,]))
  colnames(scoredf) <- c("TotalScore", "TotalDispersion")
  
  pVals <- getPvals(nullResult, scoredf)
  
  out <- list(nullResult=nullResult, pVals=pVals, scores=scoredf)
}

hallmarkSingVals <- getPvalsPathway("HALLMARK_GLYCOLYSIS", hallmarkSet, rankedGenes, hallmarkSingscores, 1000)

plotRankDensity(rankedGenes[,5,drop=FALSE], hallmarkSet[["HALLMARK_GLYCOLYSIS"]])

plotNull(permuteResult=hallmarkSingVals$nullResult, scoredf=hallmarkSingVals$scores, pvals=hallmarkSingVals$pVals, sampleNames=names(hallmarkSingVals$pVals))

testSingVal <- getPvalsPathway("REACTOME_VISUAL_PHOTOTRANSDUCTION", reactomeSet, rankedGenes, reactomeSingscores, 1000)

plotNull(permuteResult=testSingVal$nullResult, scoredf=testSingVal$scores, pvals=testSingVal$pVals, sampleNames=names(testSingVal$pVals))

plotRankDensity(rankedGenes[,8,drop=FALSE], reactomeSet[["REACTOME_VISUAL_PHOTOTRANSDUCTION"]])
...
```
## Try out PLS for singscores
```{r}
colMatrxPCKP <- as.data.frame(colData(summedRaw_tmmPCKP)) %>% left_join(flimParams, by = "sample", suffix=c("",".y")) %>% dplyr::select(-ends_with(".y"))

seletedFlimParams <- subset(colMatrxPCKP, select = c(LT2mean, LT3mean))

hallmarkSingscore.spls <- spls(t(hallmarkSingscores$Scores), seletedFlimParams,  mode = "regression", scale = TRUE, ncomp = 2, keepX = 50, keepY = 2)

variableImportance <- as.data.frame(vip(hallmarkSingscore.spls)) %>% arrange(desc(comp1))
head(variableImportance, n = 50)

plotIndiv(hallmarkSingscore.spls, group=colMatrxPCKP$Group, legend = TRUE)
#plotLoadings(hallmarkSingscore.spls, method = 'mean', contrib = 'max')
```

```{r}
hallmarkSingscore.splsda <- splsda(t(hallmarkSingscores$Scores), as.factor(colMatrxPCKP$Group), keepX = 50, scale = TRUE, ncomp=2)

hallmarkSingscore.splsda$prop_expl_var

plotLoadings(hallmarkSingscore.splsda, method = 'mean', contrib = 'max')  

variableImportanceGroup <- as.data.frame(vip(hallmarkSingscore.splsda)) %>% arrange(desc(comp1))
head(variableImportanceGroup, n = 50)

reactomeSingscore.splsda <- splsda(t(reactomeSingscores$Scores), as.factor(colMatrxPCKP$Group), keepX = 50, scale = TRUE, ncomp=2)

reactomeSingscore.splsda$prop_expl_var

plotLoadings(reactomeSingscore.splsda, method = 'mean', contrib = 'max', name.var = substr(colnames(t(reactomeSingscores$Scores)),10,25))

variableImportanceGroup <- as.data.frame(vip(reactomeSingscore.splsda)) %>% arrange(desc(comp1))
head(variableImportanceGroup, n = 50)
```

# Data Export Step
```{r}
#write gene-level DE to file
write.csv(de_results_BvT, file = "Report_Results/fullDeResults.csv")

#write gene set results to files
write.csv(roastHallmarkResult %>% arrange(FDR.Mixed), file = "Report_Results/roastHallmarkResult.csv")
write.csv(roastBioCartaResult %>% arrange(FDR.Mixed), file = "Report_Results/roastBioCartaResult.csv")
write.csv(roastReactomeResult %>% arrange(FDR.Mixed), file = "Report_Results/roastReactomeResult.csv")
write.csv(roastWikiPathwaysResult %>% arrange(FDR.Mixed), file = "Report_Results/roastWikiPathwaysResult.csv")

write.csv(cameraHallmarkResult %>% arrange(FDR), file = "Report_Results/cameraHallmarkResult.csv")
write.csv(cameraBioCartaResult %>% arrange(FDR), file = "Report_Results/cameraBioCartaResult.csv")
write.csv(cameraReactomeResult %>% arrange(FDR), file = "Report_Results/cameraReactomeResult.csv")
write.csv(cameraWikiPathwaysResult %>% arrange(FDR), file = "Report_Results/cameraWikiPathwaysResult.csv")
